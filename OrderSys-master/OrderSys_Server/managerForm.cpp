...
void ManagerForm::showBills(int option, QString billId)
{
    ...
    case 1:{//当日账单
        //sql = QString("select * from bills where  DATEDIFF(time(), billid) = 0");
        QString str = billId.mid(0, 8);//取字符串前八位
        sql = QString("select * from bills where billid like '%1%' ").arg(billId);
        break;
    }
    case 2: {//当月账单
        //sql = QString("select * from bills where  DATEDIFF(now(), billid) <= 31");
        QString str = billId.mid(0, 6);//取字符串前六位
        sql = QString("select * from bills where billid like '%1%' ").arg(billId);
        break;
    }
    case 3:{//按单号查找
        sql = QString("select * from bills where billid = %1").arg(billId);
        break;
    }
    default: break;
    }
    ...
    if(!ret){
        QSqlError err = query.lastError();   //得到最近一次的错误信息
        QString str = QString("manger122错误信息:%1, %2").arg(err.driverText()).arg(err.databaseText());
        qDebug()<< str << endl;
    }
    ...
    while(query.next()){
        ...
        if(!ret){
            ...
            QString str = QString("manger138错误信息:%1, %2").arg(err.driverText()).arg(err.databaseText());
            qDebug()<< str << endl;
        }
        if(query2.next()){
        ...
        QTableWidgetItem *item =  new QTableWidgetItem(query.value(0).toString().mid(0, 8));
...
void ManagerForm::on_pushButton_2_clicked()
{
    ...
    if(QMessageBox::Yes == ret){
        ...
        if(!ret){
            QSqlError err = query.lastError();   //得到最近一次的错误信息
            QString str = QString("manger206错误信息:%1, %2").arg(err.driverText()).arg(err.databaseText());
            qDebug()<< str << endl;
        }
...
void ManagerForm::on_pushButton_7_clicked()
{
    ...
    DBHelper helper;
    QSqlQuery query(helper.getDB());
    QString sql = QString("delete from menu where dishid = %1").arg(dishId);
    bool ret = query.exec(sql);
    if(!ret){
        QSqlError err = query.lastError();   //得到最近一次的错误信息
        QString str = QString("manger260错误信息:%1, %2").arg(err.driverText()).arg(err.databaseText());
        qDebug() << str;
...
void ManagerForm::on_pushButton_8_clicked()
{
    ...
    DBHelper helper;
    QSqlQuery query(helper.getDB());
    QString sql = QString("update menu set status = 1 where dishid = %1").arg(dishId);
    bool ret = query.exec(sql);
    if(!ret){
        QSqlError err = query.lastError();   //得到最近一次的错误信息
        QString str = QString("manger283错误信息:%1, %2").arg(err.driverText()).arg(err.databaseText());
        QMessageBox::warning(this, "提示", str);
    }
...
void ManagerForm::on_pushButton_9_clicked()
{
    ...
    DBHelper helper;
    QSqlQuery query(helper.getDB());
    QString sql = QString("update menu set status = 0 where dishid = %1").arg(dishId);
    bool ret = query.exec(sql);
    if(!ret){
        QSqlError err = query.lastError();   //得到最近一次的错误信息
        QString str = QString("manger304错误信息:%1, %2").arg(err.driverText()).arg(err.databaseText());
        QMessageBox::warning(this, "提示", str);
    }
...
void ManagerForm::on_pushButton_10_clicked()
{
    User a(1,Manager,QString("骆小云"));
    CashierForm *From = new CashierForm(a);
    From->show();

}

void ManagerForm::on_pushButton_11_clicked()
{

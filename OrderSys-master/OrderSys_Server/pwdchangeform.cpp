...
void pwdChangeForm::on_pushButton_clicked()
{
    ...
    QString sql = QString("select password from user where id = %1").arg(userId);
    bool ret = query.exec(sql);
    if(!ret){
        QSqlError err = query.lastError();   //得到最近一次的错误信息
        QString str = QString("pwd42错误信息:%1, %2").arg(err.driverText()).arg(err.databaseText());
    ...
    QString sql2 = QString("update user set password = '%1' where id = %2").arg(pwd_new).arg(userId);
    ret = query.exec(sql2);
    if(!ret){
        QSqlError err = query.lastError();   //得到最近一次的错误信息
        QString str = QString("pwd55错误信息:%1, %2").arg(err.driverText()).arg(err.databaseText());
        QMessageBox::warning(this, "提示", str);
        return ;
    }

...
void ServerThread::addDishTask(MessageFromClient mess)
{
    ...
    if(dishes.size() > 0){
        QString sql = QString("insert into orders values('%3', %1, %2, 0, '')").arg(tableId).arg(dishes[0].id).arg(QString(QTime::currentTime().toString("hh::mm::ss")));
//        qDebug() << "tableID:";
//        qDebug() << tableId;
        //QString sql = QString("insert into orders values(%3, %1, %2, 0, '')").arg(tableId).arg(dishes[0].id);
        bool ret = query.exec(sql);
        if(!ret){
            QSqlError err = query.lastError();   //得到最近一次的错误信息
            QString str = QString("serTh68错误信息:%1, %2").arg(err.driverText()).arg(err.databaseText());
            qDebug()<< str << endl;
        }
    }
...
void ServerThread::subDishTask(MessageFromClient mess)
{
    ...
    QSqlQuery query(helper.getDB());
    int orderId;
    QString sql = QString("select orderid from orders where tableid = %1 and dishid = %2 limit 1").arg(tableId).arg(dish.id);
    bool ret = query.exec(sql);
    if(!ret){
        QSqlError err = query.lastError();   //得到最近一次的错误信息
        QString str = QString("serTh87错误信息:%1, %2").arg(err.driverText()).arg(err.databaseText());
        qDebug()<< str << endl;
    }
    if(query.next()){
        orderId = query.value(0).toInt();
    }
    QString sql2  = QString("delete from orders where orderid = %1").arg(orderId);
    ret = query.exec(sql2);
    if(!ret){
        QSqlError err = query.lastError();   //得到最近一次的错误信息
        QString str = QString("serTh97错误信息:%1, %2").arg(err.driverText()).arg(err.databaseText());
        qDebug()<< str << endl;
    }
...
void ServerThread::busyDishTask(MessageFromClient mess)
{
    ...
    if(!ret){
        QSqlError err = query.lastError();   //得到最近一次的错误信息
        QString str = QString("serTh117错误信息:%1, %2").arg(err.driverText()).arg(err.databaseText());
        qDebug()<< str << endl;
    }
    emit requestReport(tableId);
}
...
void ServerThread::requestMenuTask(MessageFromClient mess)
{
     ...
     if(!ret){
         QSqlError err = query.lastError();   //得到最近一次的错误信息
         QString str = QString("serTh132错误信息:%1, %2").arg(err.driverText()).arg(err.databaseText());
         qDebug()<< str << endl;
     }
     while(query.next()){
         Dish dish;
...
void ServerThread::requestOrderTask(MessageFromClient mess)
{
    ...
    if(!ret){
        QSqlError err = query.lastError();   //得到最近一次的错误信息
        QString str = QString("serTh156错误信息:%1, %2").arg(err.driverText()).arg(err.databaseText());
        qDebug()<< str << endl;
    }
    while(query.next()){
        Dish dish;
        ...
        if(!ret){
            QSqlError err = query.lastError();   //得到最近一次的错误信息
            QString str = QString("serTh166错误信息:%1, %2").arg(err.driverText()).arg(err.databaseText());
            qDebug()<< str << endl;
        }
        if(query2.next()){
...
void ServerThread::addCommentTask(MessageFromClient mess)
{
    ...
    if(!ret){
        QSqlError err = query.lastError();   //得到最近一次的错误信息
        QString str = QString("serTh217错误信息:%1, %2").arg(err.driverText()).arg(err.databaseText());
        qDebug()<< str << endl;
    }
    emit requestReport(tableId);
}

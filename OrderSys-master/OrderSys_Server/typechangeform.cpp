...
void typeChangeForm::on_pushButton_clicked()
{
    ...
    if(type > 0){
        ...
        QString sql = QString("update user set usertype = %1 where id = %2").arg(type-1).arg(userId);
        bool ret = query.exec(sql);
        if(!ret){
            QSqlError err = query.lastError();   //得到最近一次的错误信息
            QString str = QString("typech34错误信息:%1, %2").arg(err.driverText()).arg(err.databaseText());
